# experiments/07_ukft_bianconi_entropic_gravity.py
"""
UKFT Bianconi Entropic Gravity — RELATIVE ENTROPY EDITION 🌌

Key Upgrade: "Bianconi/Araki Relative Entropy Bias"
Instead of simple gradient ascent on ρ, we minimize the quantum relative entropy 
between the vacuum geometry and the matter-dressed state.

Force Law:
F ∝ -∇(S_rel) ≈ ∇(ln ρ) = (1/ρ) * ∇ρ

This creates a self-regulating "harmonic" interaction where the pull is scaled
by the inverse density, making the "information cost" of separation significantly
more sensitive at distance (where ρ is small) while preventing singularity at the core.

Also includes a toy "G-Field" (Lambda) expansion term.
"""
...
alpha_entropic = 2.0       # Scaled down because 1/rho boosts the effective force
lambda_cosmo = 0.05        # "G-field" toy expansion (positive Lambda)
m_heavy = 10.0             
m_test = 0.1               
dt = 0.01                  
n_steps = 2000             
...
# Test particle: Start "off-axis" to get interesting chaos
pos_test = np.array([0.1, 2.5, 0.2])   

vel_h1 = np.zeros(3) # Not used (prescribed)
vel_h2 = np.zeros(3) # Not used (prescribed)
...
for step in range(n_steps):
    ...
    new_p1 = np.array([
        ...
        0.05 * np.sin(3*angle) # Small z oscillation for style
    ])
    ...
    # Knowledge density ρ (sources are the PREVIOUS positions for causality, or current? 
    # Let's use current (new_p1) as the field updates instantaneously in this toy model)
    ...
    # Acceleration = alpha * ∇(ln ρ) + Lambda * r
    eps_rho = 1e-9
    acc_t = alpha_entropic * grad_t / (rho_t + eps_rho)
    
    # "G-Field" Expansion
    acc_t += lambda_cosmo * pt
    
    # Damping (optional, creates stable orbits vs scattering)
    vel_test *= 0.995 
    
    # Euler update for Test Particle
    vel_test += acc_t * dt
...
fig = create_3d_entropic_animation(
    frames, 
    title="UKFT Bianconi/Araki Entropic Gravity — Relative Entropy Force + Emergent Lambda"
)

# SNAPSHOT: Use the LAST frame for the PNG to show trails and structure
print("Generating static preview (using final frame)...")
original_data = fig.data
fig.update(data=frames[-1].data)

output_file = os.path.join("results", "07_ukft_bianconi_entropic_gravity.html")
...
print(f"UKFT Bianconi simulation complete. Saved to {output_file}")
