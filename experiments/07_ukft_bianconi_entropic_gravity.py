...
for step in range(n_steps):
    ...
    # This mimics the "Tr(g ln G)" relative entropy minimization + expanding universe
    
    # 1. Relative Entropy Force: F ~ grad(rho) / rho
    # Add epsilon to rho to avoid division by zero at infinity
    eps_rho = 1e-9
    
    acc1 = alpha_entropic * grad1 / (rho1 + eps_rho)
    acc2 = alpha_entropic * grad2 / (rho2 + eps_rho)
    acc_t = alpha_entropic * grad_t / (rho_t + eps_rho)
    
    # 2. "G-Field" Expansion (Lambda > 0)
    # Pushes everything outward slightly, countering the strong entropic pull
    acc1 += lambda_cosmo * p1
    acc2 += lambda_cosmo * p2
