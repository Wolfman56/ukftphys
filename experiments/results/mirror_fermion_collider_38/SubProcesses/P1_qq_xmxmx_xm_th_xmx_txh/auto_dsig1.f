...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > t h t~ h
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > t h t~ h
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > t h t~ h
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION D1(VECSIZE_MEMMAX),U1(VECSIZE_MEMMAX)
     $ ,S1(VECSIZE_MEMMAX)
      DOUBLE PRECISION SX2(VECSIZE_MEMMAX),UX2(VECSIZE_MEMMAX)
     $ ,DX2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA D1,U1,S1/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
      DATA SX2,UX2,DX2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
     $ ,VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
