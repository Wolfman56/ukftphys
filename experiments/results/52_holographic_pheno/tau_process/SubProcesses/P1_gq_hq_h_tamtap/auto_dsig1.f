...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u > ta- ta+ u
      PD(IPROC)=G1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c > ta- ta+ c
      PD(IPROC)=G1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > ta- ta+ d
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > ta- ta+ s
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C     
      ...
      DOUBLE PRECISION D2(VECSIZE_MEMMAX),U2(VECSIZE_MEMMAX)
     $ ,S2(VECSIZE_MEMMAX),C2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     
      ...
      DATA D2,U2,S2,C2/VECSIZE_MEMMAX*1D0,VECSIZE_MEMMAX*1D0
