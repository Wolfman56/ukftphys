...
class UKFTVolumeLens:
    def __init__(self, resolution=200, stars=5000, frames=60):
        self.res = resolution
        self.frames = frames
        self.box_size = 20.0 # Cube size [-10, 10]
        self.R_s = 1.5       # Schwarzschild Radius
        self.LENS_STRENGTH = 15.0 # Distortion factor
    ...
    def trace_rays(self, bh_pos):
        ...
        visible_outer = dist_outer > self.R_s
        visible_inner = (dist_inner > self.R_s) & mask_behind_lens # Only background stars get secondary image
        ...
        # Draw Outer
        px, py = to_pix(im_x_outer, im_y_outer)
        # Bounds check
        valid = (px >= 0) & (px < self.res) & (py >= 0) & (py < self.res) & visible_outer
        
        # Add color
        # We use a simple additive blend for stars
        # Flatten accumulation
        for i in np.where(valid)[0]:
            image[py[i], px[i]] += self.star_colors[i]
            
        # Draw Inner (Secondary Images - The "Ring")
        px, py = to_pix(im_x_inner, im_y_inner)
        valid = (px >= 0) & (px < self.res) & (py >= 0) & (py < self.res) & visible_inner
        
        for i in np.where(valid)[0]:
             # Inner images are often dimmer or distorted. Let's keep brightness for visibility.
             image[py[i], px[i]] += self.star_colors[i]
             
        # Normalize
        image = np.clip(image, 0, 1)
        
        # Draw UKFT Mirror (The Sphere itself)
        # Simple rasterization of the sphere at BH pos
        bx, by = to_pix(np.array([bh_pos[0]]), np.array([bh_pos[1]]))
        rr, cc = np.ogrid[:self.res, :self.res]
        # Distance squared from BH center in pixels
        r2 = (rr - by)**2 + (cc - bx)**2
        radius_pix = (self.R_s / self.box_size * self.res)
        
        mask_bh = r2 < radius_pix**2
        
        # Mirror Shader (Cyan/Metallic)
        # Gradient based on distance from center
        norm_r = np.sqrt(r2[mask_bh]) / radius_pix
        shader = 1.0 - norm_r**2
        
        image[mask_bh, 0] = shader * 0.0
        image[mask_bh, 1] = shader * 0.8
        image[mask_bh, 2] = shader * 1.0
    ...
    def animate(self):
        ...
        def update(frame):
            ...
            # Parametric Path for BH (Spiral? Line?)
            # Let's do a Z-depth traversal + slight XY wiggle
            t = frame / self.frames # 0 to 1
            z = -10.0 + 40.0 * t  # Moves from -10 to 30 (Through the star field)
            x = 3.0 * np.sin(2 * np.pi * t)
            y = 2.0 * np.cos(2 * np.pi * t)
            
            bh_pos = np.array([x, y, z])
            
            img_data = self.trace_rays(bh_pos)
            im.set_data(img_data)
            ax.set_title(f"UKFT Volumetric Lensing\nBH Depth Z={z:.1f}", color='white')
            return [im]
